"""Der CART-Baumkern (aus cart-demo/random-forest-demo übernommen) - um `mtry` (wie Random Forest) UND `extra` (Extra Trees) erweitert. `extra=False` sucht wie gehabt die beste Schwelle über alle Werte
(`best_split`, exakter Kern von cart-demo/bagging-demo/random-forest-demo, unverändert). `extra=True` zieht für jedes Kandidatenmerkmal genau EINE zufällige Schwelle statt sie zu suchen (`random_split`) -
das ist Extra Trees (Geurts, Ernst, Wehenkel 2006): pro Knoten `len(candidates)` geprüfte (Merkmal, Schwelle)-Paare statt `len(candidates) × (m - 1)` bei `m` Zeilen im Knoten (Kreuzprobe: Zähler exakt vergleichbar).
Mit `mtry=None, extra=False` verhält sich `grow` exakt wie in cart-demo/bagging-demo (Kreuzprobe: mtry = alle Merkmale == Bagging); mit `extra=False, mtry<d` wie random-forest-demo.
...
def random_split(X, y, criterion, min_leaf, candidates, rng):
    """Extra Trees: für jedes Merkmal in `candidates` genau eine zufällige Schwelle (gleichverteilt zwischen dem kleinsten und größten Wert dieses Merkmals im Knoten) statt einer Suche über alle Schwellen.
    Der beste der geprüften Zufallsschnitte gewinnt; bei Gleichstand das kleinste Merkmal (Reihenfolge von `candidates`, muss aufsteigend sortiert sein). None, wenn kein Kandidat einen zulässigen Schnitt ergibt."""
    if len(y) < 2 * min_leaf or len(y) < 2:
        return None
    parent = node_impurity(y, criterion)
    n = len(y)
    best = None
    for f in candidates:
        v = X[:, f]
        ...
        if hi <= lo:
            continue                                                         # im Knoten konstant - kein Schnitt möglich
        ...
        if nl < min_leaf or nr < min_leaf:
            continue
        gain = parent - (nl * node_impurity(y[go_left], criterion) + nr * node_impurity(y[~go_left], criterion)) / n
...
def grow(X, y, task="class", criterion=None, max_depth=None, min_leaf=1, mtry=None, seed=0, extra=False):
    ...
    `mtry` (Random Forest): an jedem Knoten werden nur `mtry` zufällig gezogene Merkmale geprüft; `None` oder `mtry >= d` prüft alle. `extra` (Extra Trees): pro geprüftem Merkmal nur eine zufällige Schwelle
    statt der besten (siehe `random_split`) - dann werden IMMER alle `mtry` (bzw. alle d) Merkmale als Kandidaten gezogen, auch ohne `mtry`, weil die Zufallsschwelle selbst schon die Randomisierung ist."""
    ...
    rng = np.random.default_rng(seed) if (use_mtry or extra) else None
    ...
    while queue:
        ...
        if depth[t] >= max_depth or imp[t] <= EPS:
            continue
        candidates = np.sort(rng.choice(d, mtry, replace=False)) if use_mtry else (np.arange(d) if extra else None)
        s = random_split(X[idx], y[idx], criterion, min_leaf, candidates, rng) if extra else best_split(X[idx], y[idx], criterion, min_leaf, candidates)
        if s is None:
            continue
        f, thr, _ = s
        go_left = X[idx, f] <= thr
