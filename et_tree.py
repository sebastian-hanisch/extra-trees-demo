...
def random_split(X, y, criterion, min_leaf, candidates, rng):
    ...
    Der beste der geprüften Zufalls-Splits gewinnt; bei Gleichstand das kleinste Merkmal (Reihenfolge von `candidates`, muss aufsteigend sortiert sein). None, wenn kein Kandidat einen zulässigen Split ergibt."""
    if len(y) < 2 * min_leaf or len(y) < 2:
        return None
    ...
    for f in candidates:
        ...
        if hi <= lo:
            continue                                                         # im Knoten konstant - kein Split möglich
